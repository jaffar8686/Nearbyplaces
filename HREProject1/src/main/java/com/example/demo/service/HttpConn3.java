...
public class HttpConn3 {
	

	
	private static HttpURLConnection conn;
	private static final java.util.logging.Logger log = java.util.logging.Logger.getLogger(HttpConn3.class.getName());
	...
	public HttpConn3(RestaurantRepository restaurantRepository,String latitude,String longitude) {
	...
	public  ArrayList parse(String responseBody) {
		...
		List<Restaurant> nearPlaces=new ArrayList<>(); 
	   ...
	   // hotel.setLocality(locality);
	    ...
	    restaurant.setPost_code(post_code);
	    restaurant.setTimezone(timezone);
	    restaurant.setRegion(region);
	    restaurant.setCountry(country);
	    restaurant.setCross_street(cross_street);
	    nearPlaces.add(restaurant);
	 ...
	 public  ArrayList getPlaces() {
		...
		try{
		URL url = new URL("https://api.foursquare.com/v3/places/nearby?ll="+lat+"%2C"+lon+"&query=restaurant");
